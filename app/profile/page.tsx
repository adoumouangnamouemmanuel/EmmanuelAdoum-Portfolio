...
export default async function ProfilePage({
  ...
  const basePath = isFr ? "/fr" : "";
  const dateLocale = isFr ? "fr-FR" : "en-US";
  const t = isFr
    ? {
        returnHome: "Retour à l'accueil",
        profileTitlePrefix: "Votre",
        profileTitleHighlight: "Profil.",
        admin: "Admin",
        user: "Utilisateur",
        anonymousId: "Identité anonyme",
        ...
        noPostsDesc: "Vous n'avez publié aucun article pour le moment.",
        createPost: "Créer un article",
        noCommentsYet: "Aucun commentaire pour le moment",
        noCommentsDesc:
          "Vous n'avez publié aucun commentaire sur le blog pour le moment.",
        readBlog: "Lire le blog",
        commentedOn: "Commenté sur",
        noRepliesYet: "Aucune réponse pour le moment",
        noRepliesDesc:
          "Vous n'avez répondu à aucun commentaire pour le moment.",
        repliedOn: "Répondu sur",
        postPrefix: "Article :",
        untitledDocument: "Document sans titre",
      }
    : {
        returnHome: "Return Home",
        profileTitlePrefix: "Your",
        profileTitleHighlight: "Profile.",
        admin: "Admin",
        user: "User",
        ...
        noPostsDesc: "You have not published any posts yet.",
        createPost: "Create Post",
        noCommentsYet: "No Comments Yet",
        noCommentsDesc: "You haven't posted any comments on the blog yet.",
        readBlog: "Read Blog",
        commentedOn: "Commented On",
        noRepliesYet: "No Replies Yet",
        noRepliesDesc: "You haven't replied to any comments yet.",
  ...
  try {
    ...
    const postIds = new Set(
      commentsSnapshot.docs.map((doc) => doc.data().postId),
    );
    ...
    for (const postId of postIds) {
      ...
      if (postDoc.exists) {
        ...
        postsMap.set(postId, {
          title: postData?.title || t.untitledDocument,
    ...
    const allComments = commentsSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
      postTitle: postsMap.get(doc.data().postId)?.title || t.untitledDocument,
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 selection:bg-blue-200 dark:selection:bg-blue-900/50 pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-16">
        ...
        <div className="mb-12">
          <Link
            href=".."
            className="inline-flex items-center text-[10px] font-bold tracking-[0.2em] uppercase text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors group"
          >
            <ArrowLeft className="mr-3 h-4 w-4 group-hover:-translate-x-2 transition-transform duration-300" />
            {t.returnHome}
          </Link>
          <h1 className="text-4xl sm:text-6xl font-bold tracking-tighter text-slate-900 dark:text-white mt-8 mb-4">
            {t.profileTitlePrefix}{" "}
            <span className="text-blue-600 dark:text-blue-400">
              {t.profileTitleHighlight}
            </span>
          </h1>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          ...
          <aside className="w-full lg:w-4/12 xl:w-3/12 relative">
            <div className="sticky top-32 flex flex-col gap-8">
              ...
              <div className="relative overflow-hidden rounded-[2.5rem] bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 p-8 sm:p-10 flex flex-col items-center text-center shadow-xl">
                ...
                <div className="z-10 w-full flex flex-col items-center">
                  ...
                  <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white mb-3">
                    {userData?.name || session.user.name || t.anonymousId}
                  </h2>
                  ...
                  {userData?.github && (
                    <div className="flex items-center gap-4 group">
                      ...
                      <a
                        ...
                        className="text-xs font-medium text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors truncate"
                      >
              ...
              <Link
                href="/settings"
                className="group relative flex items-center justify-between p-6 rounded-3xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden"
              >
                ...
                (posts.length === 0 ? (
                  <div className="py-24 text-center border border-dashed border-slate-300 dark:border-slate-800 rounded-[2.5rem]">
                    ...
                    <Link
                      ...
                      className="inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[10px] font-bold tracking-[0.2em] uppercase hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
                    >
                      {t.createPost}
              ...
              {activeTab === "comments" &&
                (comments.length === 0 ? (
                  <div className="py-24 text-center border border-dashed border-slate-300 dark:border-slate-800 rounded-[2.5rem]">
                    ...
                    <Link
                      href={`${basePath}/blog`}
                      className="inline-flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 text-[10px] font-bold tracking-[0.2em] uppercase hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors"
                    >
                      {t.readBlog}
                    </Link>
                  </div>
                  ...
                  comments.map((comment) => (
                    <Link
                      key={comment.id}
                      href={`${basePath}/blog/${comment.postSlug}`}
                      className="group block"
                    >
              ...
              {activeTab === "replies" &&
                (replies.length === 0 ? (
                  ...
                  replies.map((reply) => (
                    <Link
                      key={reply.id}
                      href={`${basePath}/blog/${reply.postSlug}`}
                      className="group block ml-0 sm:ml-12 relative"
                    >
                      ...
                      <div className="bg-slate-100 dark:bg-slate-900/60 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 flex flex-col lg:flex-row gap-6 sm:items-start justify-between">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col gap-1 mb-4">
                            <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-indigo-600 dark:text-indigo-400">
                              {t.repliedOn}
                            </span>
                            <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 truncate opacity-60">
                              {t.postPrefix} {reply.postTitle}
                            </h4>
                          </div>
