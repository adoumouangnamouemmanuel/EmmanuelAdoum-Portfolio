...
function SettingsForm({ user }: { user: any }) {
  ...
  const cropBounds = useMemo(() => {
    if (!imageNaturalSize) {
      return { maxX: 0, maxY: 0, previewScale: 1 };
    }

    const baseScale = Math.max(
      CROPPER_SIZE / imageNaturalSize.width,
      CROPPER_SIZE / imageNaturalSize.height,
    );
    const previewScale = baseScale * cropZoom;
    const displayWidth = imageNaturalSize.width * previewScale;
    const displayHeight = imageNaturalSize.height * previewScale;

    return {
      maxX: Math.max(0, (displayWidth - CROPPER_SIZE) / 2),
      maxY: Math.max(0, (displayHeight - CROPPER_SIZE) / 2),
      previewScale,
    };
  }, [imageNaturalSize, cropZoom]);

  useEffect(() => {
    setCropX((prev) =>
      Math.max(-cropBounds.maxX, Math.min(cropBounds.maxX, prev)),
    );
    setCropY((prev) =>
      Math.max(-cropBounds.maxY, Math.min(cropBounds.maxY, prev)),
    );
  }, [cropBounds.maxX, cropBounds.maxY]);
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Allow larger originals; server will optimize and enforce final limits.
  ...
  const handleCropAndUpload = async () => {
    ...
    setIsLoading(true);
    try {
      const image = await loadImageElement(selectedImageSrc);
      const baseScale = Math.max(
        CROPPER_SIZE / image.width,
        CROPPER_SIZE / image.height,
      );
      const previewScale = baseScale * cropZoom;
      ...
      const clampedX = Math.max(
        -Math.max(0, (displayWidth - CROPPER_SIZE) / 2),
        Math.min(Math.max(0, (displayWidth - CROPPER_SIZE) / 2), cropX),
      );
      const clampedY = Math.max(
        -Math.max(0, (displayHeight - CROPPER_SIZE) / 2),
        Math.min(Math.max(0, (displayHeight - CROPPER_SIZE) / 2), cropY),
      );

      const imageX = (CROPPER_SIZE - displayWidth) / 2 + clampedX;
      const imageY = (CROPPER_SIZE - displayHeight) / 2 + clampedY;

      const sourceX = Math.max(0, (0 - imageX) / previewScale);
      const sourceY = Math.max(0, (0 - imageY) / previewScale);
      const sourceSize = Math.min(
        image.width - sourceX,
        image.height - sourceY,
        CROPPER_SIZE / previewScale,
      );
              ...
              >
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        strokeWidth={2}
                        d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.5a6.002 6.002 0 00-4 5.659v3.159a2.032 2.032 0 01-.595 1.436L6 17H1m5-11v1a1 1 0 001 1h3.5a1 1 0 001-1v-1a1 1 0 00-1-1H7a1 1 0 00-1 1z"
                      />
                    </svg>
                    Notifications
                  </TabsTrigger>
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground mt-2">
                        Click to crop manually. Final image is auto-optimized to
                        512x512 and max 5MB.
                      </p>
                    </motion.div>
                    ...
                    >
                      <DialogContent className="sm:max-w-md">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Drag and zoom your image to frame it before upload.
                          </DialogDescription>
                        </DialogHeader>

                        {selectedImageSrc ? (
                          <div className="space-y-4">
                            <div className="mx-auto relative h-[280px] w-[280px] rounded-full overflow-hidden border border-gray-200 dark:border-gray-700 bg-black">
                              <div
                                ...
                                onMouseMove={(event) => {
                                  if (!isDraggingCrop || !dragStartPoint)
                                    return;

                                  const nextX =
                                    dragStartPoint.cropX +
                                    (event.clientX - dragStartPoint.x);
                                  const nextY =
                                    dragStartPoint.cropY +
                                    (event.clientY - dragStartPoint.y);

                                  setCropX(
                                    Math.max(
                                      -cropBounds.maxX,
                                      Math.min(cropBounds.maxX, nextX),
                                    ),
                                  );
                                  setCropY(
                                    Math.max(
                                      -cropBounds.maxY,
                                      Math.min(cropBounds.maxY, nextY),
                                    ),
                                  );
                                }}
                                onMouseUp={() => {
                              ...
                              <img
                                ...
                                style={{
                                  ...
                                  top: "50%",
                                  maxWidth: "none",
                                  transform: `translate(calc(-50% + ${cropX}px), calc(-50% + ${cropY}px)) scale(${cropBounds.previewScale})`,
                        ...
                        <DialogFooter>
                          ...
                          <Button
                            type="button"
                            disabled={isLoading || !selectedImageSrc}
                ...
                <TabsContent value="notifications">
                  <motion.div
                    ...
                    initial="hidden"
                    animate={
                      activeTab === "notifications" ? "visible" : "hidden"
                    }
                  >
                    <motion.div
