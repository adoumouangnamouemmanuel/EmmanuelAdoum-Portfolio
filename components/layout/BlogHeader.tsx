...
import { LogOut, Menu, Moon, Settings, Shield, Sun, User, X, Home } from "lucide-react"
                    ...
                    >
                      <Avatar className="h-full w-full border border-slate-200 dark:border-slate-800">
                        ...
                        <AvatarFallback className="bg-slate-900 text-white font-bold text-xs uppercase">
                          {session.user.name ? session.user.name.substring(0,2) : "ID"}
                        </AvatarFallback>
                      </Avatar>
                    </Button>
                  </DropdownMenuTrigger>
                  ...
                  >
                    <div className="flex items-center justify-start gap-4 p-4 border-b border-slate-200/50 dark:border-slate-800/50 mb-2">
                      <Avatar className="h-12 w-12 border border-slate-200 dark:border-slate-800">
                        <AvatarImage src={session.user.image || "/images/posts/profile.jpeg"} alt={session.user.name || "User"} />
                        <AvatarFallback className="bg-slate-900 text-white">ID</AvatarFallback>
                      </Avatar>
                      <div className="flex flex-col">
                        <p className="font-bold text-slate-900 dark:text-white tracking-tight">{session.user.name}</p>
                        <p className="text-xs text-slate-500 font-mono break-all">{session.user.email}</p>
                      </div>
                    </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
             ...
             <div className="absolute top-6 left-6 sm:hidden">
                <Button variant="outline" size="icon" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="rounded-full w-10 h-10 border-slate-200 dark:border-slate-800 bg-transparent">
