...
export default function BlogSection() {
  ...
  return (
    <section id="articles" className="py-24 lg:py-32 relative overflow-hidden bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-16 mb-16 lg:mb-24 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
        <div>
          ...
          >
            Latest <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400">Articles.</span>
          </motion.h2>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-16" ref={ref}>
        {loading ? (
          ...
          <div className="w-full py-32 rounded-3xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-center">
            <h3 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white mb-4">No broadcasts available.</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
            ...
            {posts[0] && (
               ...
               >
                 <Link href={`/blog/${posts[0].slug}`} className="absolute inset-0 z-20" aria-label={`Read ${posts[0].title}`} />
                 
                 <motion.div style={{ y: y1, scale: 1.15 }} className="absolute inset-0 w-full h-[120%] z-0">
                    <Image
                 ...
                 <div className="relative z-10 p-8 sm:p-12 h-full flex flex-col justify-end pointer-events-none">
                    <div className="flex flex-wrap gap-2 sm:gap-3 mb-6">
                       {(posts[0].categories || []).map((cat: string, i: number) => (
            ...
            <div className="lg:col-span-5 flex flex-col gap-6 sm:gap-8 h-full">
                        ...
                        <div>
                           <div className="flex flex-wrap gap-2 mb-4">
                              {(post.categories || []).slice(0, 2).map((cat: string, i: number) => (
                                 <span key={i} className="text-[9px] sm:text-[10px] font-bold tracking-widest uppercase text-blue-600 dark:text-blue-400">
                                    {cat}
                                    {i === 0 && post.categories.length > 1 && <span className="ml-2 text-slate-300 dark:text-slate-600">·</span>}
                                 </span>
                              ))}
                           </div>
               ...
               {posts.length === 2 && (
                  <div className="flex-1 rounded-[2rem] bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 border-dashed flex items-center justify-center min-h-[350px]">
                     <span className="text-[10px] sm:text-xs font-bold tracking-widest uppercase text-slate-400">More articles publishing soon</span>
                  </div>
               )}
            </div>
