...
export default function AdminDashboard() {
  ...
  const [searchQuery, setSearchQuery] = useState(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("adminSearch") || ""
      : "",
  );
  ...
  const [tab, setTab] = useState(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("adminTab") || "posts"
      : "posts",
  );
  ...
  const [lastUpdated, setLastUpdated] = useState(() => {
    if (typeof window !== "undefined") {
      return (
        localStorage.getItem("adminLastUpdated") || new Date().toISOString()
      );
    }
  ...
  const avgCommentsPerPost = (
    totalComments / Math.max(posts.length, 1)
  ).toFixed(1);
  ...
  return (
    <TooltipProvider>
      <motion.div
        ...
        animate="visible"
        variants={fadeIn}
        className="relative overflow-hidden min-h-screen bg-gradient-to-br from-violet-50 via-white to-indigo-50 dark:from-gray-950 dark:via-slate-950 dark:to-indigo-950 py-8 px-4 sm:px-6 lg:px-8"
      >
        <div className="pointer-events-none absolute -top-28 -left-28 h-80 w-80 rounded-full bg-violet-400/20 blur-3xl dark:bg-violet-500/20" />
        <div className="pointer-events-none absolute -bottom-28 -right-20 h-96 w-96 rounded-full bg-indigo-400/20 blur-3xl dark:bg-indigo-500/20" />
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(124,58,237,0.08),transparent_40%),radial-gradient(circle_at_70%_65%,rgba(79,70,229,0.08),transparent_35%)] dark:bg-[radial-gradient(circle_at_30%_20%,rgba(139,92,246,0.12),transparent_40%),radial-gradient(circle_at_70%_65%,rgba(99,102,241,0.12),transparent_35%)]" />
        <div className="max-w-7xl mx-auto">
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="mb-8 rounded-3xl border border-violet-200/70 bg-white/80 p-6 shadow-xl shadow-violet-200/40 backdrop-blur-xl dark:border-violet-900/50 dark:bg-slate-900/60 dark:shadow-violet-950/30"
          >
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-violet-300/60 bg-violet-100/70 px-3 py-1 text-xs font-semibold tracking-wide text-violet-700 dark:border-violet-700/70 dark:bg-violet-900/30 dark:text-violet-300">
                  <ShieldCheck className="h-3.5 w-3.5" />
                  Admin Control Room
                </div>
                <h1 className="text-3xl sm:text-4xl font-black tracking-tight bg-gradient-to-r from-violet-700 via-fuchsia-600 to-indigo-700 dark:from-violet-300 dark:via-fuchsia-300 dark:to-indigo-300 bg-clip-text text-transparent">
                  Dashboard Pulse
                </h1>
                <p className="mt-2 text-sm text-slate-600 dark:text-slate-300 max-w-2xl">
                  Monitor publishing flow, moderate users, and orchestrate
                  content operations from one premium command center.
                </p>
                <div className="mt-4 flex flex-wrap items-center gap-3 text-xs">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-300/60 bg-emerald-50 px-3 py-1 font-medium text-emerald-700 dark:border-emerald-700/60 dark:bg-emerald-900/25 dark:text-emerald-300">
                    <Sparkles className="h-3.5 w-3.5" />
                    {publishedCount} Published
                  </span>
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-indigo-300/60 bg-indigo-50 px-3 py-1 font-medium text-indigo-700 dark:border-indigo-700/60 dark:bg-indigo-900/25 dark:text-indigo-300">
                    <Users className="h-3.5 w-3.5" />
                    {activeUsersCount} Active Users
                  </span>
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-violet-300/60 bg-violet-50 px-3 py-1 font-medium text-violet-700 dark:border-violet-700/60 dark:bg-violet-900/25 dark:text-violet-300">
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 lg:justify-end">
                <Button
                  onClick={refreshData}
                  variant="outline"
                  size="sm"
                  className="h-10 rounded-xl border-violet-300/60 bg-white/90 px-4 text-violet-700 shadow-sm hover:bg-violet-50 dark:border-violet-800 dark:bg-slate-900/70 dark:text-violet-300 dark:hover:bg-violet-900/30"
                >
                  <RefreshCw
                ...
                <Link
                  href="/"
                  className="inline-flex h-10 items-center justify-center rounded-xl bg-gradient-to-r from-fuchsia-600 to-violet-600 px-4 text-sm font-semibold text-white shadow-lg shadow-fuchsia-500/25 transition hover:from-fuchsia-700 hover:to-violet-700"
                >
                  Go to Portfolio
                </Link>
                <Link
                  href="/blog"
                  className="inline-flex h-10 items-center justify-center rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 px-4 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition hover:from-indigo-700 hover:to-blue-700"
                >
                  Go to Blogs
                </Link>
              </div>
                ...
                <CardContent className="pt-6">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    >
                      ...
                      {Math.round(
                        (publishedCount / Math.max(posts.length, 1)) * 100,
                      )}
                      %
                    </Badge>
                  </div>
          ...
          <motion.div variants={fadeIn}>
            <Tabs value={tab} onValueChange={setTab} className="w-full">
              <TabsList className="mb-6 h-auto bg-white/75 dark:bg-gray-900/65 backdrop-blur-xl border border-violet-200/70 dark:border-violet-800 p-1.5 rounded-2xl shadow-lg shadow-violet-200/35 dark:shadow-violet-950/20">
                <TabsTrigger
                  value="posts"
                  className="rounded-xl px-4 py-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-500/15 data-[state=active]:to-indigo-500/15 data-[state=active]:text-violet-700 dark:data-[state=active]:text-violet-300 data-[state=active]:shadow-sm transition-all duration-300"
                >
                ...
                <TabsTrigger
                  value="users"
                  className="rounded-xl px-4 py-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500/15 data-[state=active]:to-purple-500/15 data-[state=active]:text-indigo-700 dark:data-[state=active]:text-indigo-300 data-[state=active]:shadow-sm transition-all duration-300"
                >
                    ...
                    <Table>
                      <TableHeader className="bg-gradient-to-r from-violet-500/10 to-indigo-500/10 dark:from-violet-500/5 dark:to-indigo-500/5">
                        <TableRow className="hover:bg-transparent border-violet-200 dark:border-violet-800">
                          <TableHead className="text-violet-700 dark:text-violet-400 font-medium">
                            Title
                          </TableHead>
                          <TableHead className="text-violet-700 dark:text-violet-400 font-medium">
                            Author
                                      ...
                                      <AlertDialogContent className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-violet-200 dark:border-violet-800 shadow-xl">
                                        <AlertDialogHeader>
                                          ...
                                          <AlertDialogDescription>
                                            ...
                                            will permanently delete the post and
                    ...
                    <Table>
                      <TableHeader className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 dark:from-indigo-500/5 dark:to-purple-500/5">
                        <TableRow className="hover:bg-transparent border-indigo-200 dark:border-indigo-800">
                          <TableHead className="text-indigo-700 dark:text-indigo-400 font-medium">
                            User
                          </TableHead>
                          <TableHead className="text-indigo-700 dark:text-indigo-400 font-medium">
                            Email
